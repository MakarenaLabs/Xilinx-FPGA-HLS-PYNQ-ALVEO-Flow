...
//------------------------Parameter----------------------
`define TV_IN_a "../tv/cdatafile/c.mul_matrix.autotvin_a.dat"
`define TV_IN_b "../tv/cdatafile/c.mul_matrix.autotvin_b.dat"
`define TV_IN_c "../tv/cdatafile/c.mul_matrix.autotvin_c.dat"
...
parameter c_DEPTH = 1;
reg [31 : 0] c_OPERATE_DEPTH = 1;
parameter c_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter mul_matrix_continue_addr = 0;
parameter mul_matrix_auto_start_addr = 0;
            ...
            if (write_c_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (c_c_bitwidth < 32) begin
                        c_data_tmp_reg = reg_c;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < c_c_bitwidth) begin
                                c_data_tmp_reg[j] = reg_c[i*32 + j];
                            end
                            else begin
                                c_data_tmp_reg[j] = 0;
                            end
